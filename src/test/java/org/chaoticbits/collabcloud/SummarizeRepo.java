...
public class SummarizeRepo {
	...
	public static void main(String[] args) throws ParseException, IOException {
		...
		// IPlaceStrategy networkPlaceStrategy = new CenteredTokenWrapper(new
		// ParentNetworkPlacement(weights.tokens(), new Dimension(WIDTH / 2,
		// HEIGHT / 2), new Point2D.Double(2 * WIDTH / 3, 2 * HEIGHT / 3)));
//		IPlaceStrategy contributionNetworkPlaceStrategy = new CenteredTokenWrapper(new ContributionNetworkPlacement(weights.tokens(),
//				developers, new Dimension(WIDTH / 2, HEIGHT / 2), new Point2D.Double(2 * WIDTH / 3, 2 * HEIGHT / 3)));
//		new LayoutTokens(WIDTH, HEIGHT, FONT_TRANSFORMER, checker, contributionNetworkPlaceStrategy, spiral, COLOR_SCHEME).makeImage(weights,
