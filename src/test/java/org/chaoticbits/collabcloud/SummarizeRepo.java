...
public class SummarizeRepo {
	...
	// private static final String THIS_REPO_SECOND_COMMIT_ID =
	// "4cfde077a84185b06117bcff5d47c53644463b1f";
	private static final Random RAND = new Random();
	private static final IPlaceStrategy PLACE_STRATEGY = new CenteredTokenWrapper(new RandomPlacement(RAND, new Rectangle2D.Double(400,
			400, 50, 50)));
	...
	public static void main(String[] args) throws ParseException, IOException {
		...
		weights = new GitLoader(new File(TEST_BED.getAbsolutePath() + "/.git"), GitLoaderTest.SECOND_COMMIT_ID).crossWithDiff(weights,
				modifier);
		// CloudWeights weights = new JavaProjectSummarizer().summarize(new
		// File(THIS_REPO.getAbsolutePath()
		// + "/src"));
		// weights = new GitLoader(new File(THIS_REPO.getAbsolutePath() +
		// "/.git"),THIS_REPO_SECOND_COMMIT_ID).crossWithDiff(weights,
		// modifier);
